# VERY EARLY FIRST DRAFT AGENDA

Discovery of Agents, Workloads, and Named entities (DAWN)

BoF Meeting at IETF-126, Vienna, Austria

1. Administrivia and purpose of the BoF
      - Presenters: Chairs and ADs
      - Content: Usual meeting details. Purpose and non-purpose of BoF.
      - Time: 5 minutes (5/120)
   
2. Scene-setting, terminology, and problem statement
      - Potentinal presenter: Arashmid Akhavain (Huawei Canada)
      - Content: What are we talking about when we say "entity" and "discovery"? What is the essence of the problem to be solved?
      - References:
           - https://datatracker.ietf.org/doc/draft-farrel-dawn-terminology/
           - https://datatracker.ietf.org/doc/draft-akhavain-moussa-dawn-problem-statement/
      - Time: 15 minutes (20/120)

3. Overview of categories of use cases
      - Potential presenter: Kehan Yao (China Mobile)
      - Content: Broad-brush view of the use cases by category rather than details.
      - Reference:
           - https://datatracker.ietf.org/doc/draft-kay-dawn-use-cases/
      - Time: 10 minutes (30/120)
   
4. Solution requirements
      - Potential presenter: Dan King (Old Dog Consulting)
      - Content: What does a solution need to include?
      - Reference:
           - https://datatracker.ietf.org/doc/draft-king-dawn-requirements/
      - Time: 10 minutes (40/120)

5. Potential solution space
      - Potential presenter: Jim Mozley (Infoblox)
      - Content: Run around the houses of solutions that might be considered
      - Reference:
           - https://datatracker.ietf.org/doc/draft-moussa-dawn-gap-analysis/
           - https://github.com/danielkinguk/discovery/blob/main/solutions-list.md
      - Time: 10 minutes (50/120)

6. Draft charter
      - Potential presenter: Jim Mozley (Infoblox)
      - Content: Quick skim of the main points in the draft charter as feed to open discussion
      - Reference:
           - https://github.com/danielkinguk/discovery/blob/main/charter/dawn-charter-01.md
      - Time: 10 minutes (60/120)

7. Open discussion
      - Presenters: Chairs
      - Content: Open discussion and key questions posed by chairs
      - Time: 55 minutes (115/120)
    
8. Wrap-up / Next steps
      - Presenters: Chairs and ADs
      - Content: What was heard. Actions and next steps
      - Time: 5 minutes (120/120)
